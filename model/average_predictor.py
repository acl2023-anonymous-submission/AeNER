...
class AverageBlock(nn.Module):
    ...
    def forward(
        ...
        return AverageBlock.Result(log_probs, best_signs)
    ...
    def calc_log_marginal_likelihood(
        answer_as_avg_expressions, prediction, number_mask
    ):
        # The padded add-sub combinations use 0 as the signs for all numbers, and we mask them here.
        # Shape: (batch_size, # of combinations)
        gold_avg_mask = (answer_as_avg_expressions.sum(-1) > 0).float()
        # Shape: (batch_size, # of numbers in the passage, # of combinations)
        gold_avg_signs = answer_as_avg_expressions.transpose(1, 2)
        # Shape: (batch_size, # of numbers in the passage, # of combinations)
        log_likelihood = torch.gather(prediction.log_probs, 2, gold_avg_signs)
        ...
        log_likelihood = replace_masked_values_with_big_negative_number(
            log_likelihood, gold_avg_mask.bool()
        )
        # Shape: (batch_size, )
